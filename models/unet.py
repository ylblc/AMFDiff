        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks[level] + 1):
                ...
                self.skip_connections.append(
                    PyConv3AdaptiveSEResDP(ich,ich)
                )
                ch = int(model_channels * mult)
                if ds in attention_resolutions and i==0:
                    layers.append(
...
class AdaptiveGateFusion(nn.Module):
    def __init__(self, channels,reduction=8):
        ...
        self.gate_net = nn.Sequential(
            nn.Conv2d(2 * channels, channels // reduction, 3, padding=1),
            nn.ReLU(),
            nn.Conv2d(channels // reduction, 1, 1),
            nn.Sigmoid()
        )
        # # 在模型初始化中添加
        self.residual_scale = nn.Parameter(th.tensor(1.0))  # 初始小值
    def forward(self, skip_feat, fused_feat):
        # 计算融合门控值
        combined = th.cat([skip_feat, fused_feat], dim=1)
        gate_weight = self.gate_net(combined)
        combined_resH = skip_feat * gate_weight +  fused_feat * (1 - gate_weight)
...
class MultiScaleAttentionAFPF(AFPF):
    def __init__(self, *args, **kwargs):
        ...
        self.scale_attentions = nn.ModuleList([
            nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(self.out_channels, self.out_channels // 8, 1),
                nn.ReLU(),
                nn.Conv2d(self.out_channels // 8, 1, 1),
                nn.Sigmoid()
        ...
        self.scale_convs = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(self.out_channels, self.out_channels, 3, dilation=d, padding=d),
                nn.ReLU()
            ) for d in [1, 2, 3]  # 不同膨胀率
        ])
    ...
    def forward(self, features, base_feature):
        ...
        # 多尺度特征提取
        scale_features = [conv(fused) for conv in self.scale_convs]

        # 多尺度注意力
        scale_weights = [attn(feat) for attn, feat in zip(self.scale_attentions, scale_features)]

        # 加权融合
        enhanced = th.zeros_like(fused)
        for weight, feat in zip(scale_weights, scale_features):
            enhanced += weight * feat

        return fused + 0.1 * enhanced
class ResBlockPyConv(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
...
class ResBlockPyConvNew(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        # 沿通道维度计算均值与最大值
        avg_out = th.mean(x, dim=1, keepdim=True)  # [B,1,H,W]
        max_out, _ = th.max(x, dim=1, keepdim=True)  # [B,1,H,W]
        feat = th.cat([avg_out, max_out], dim=1)  # [B,2,H,W]
        attn = th.sigmoid(self.conv(feat))  # [B,1,H,W]
        return x * attn  # 特征加权